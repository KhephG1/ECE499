...
BME68X_INTF_RET_TYPE bme68x_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    (void)intf_ptr;
    uint8_t status = HAL_I2C_Master_Transmit(&hi2c1, device_addr<<1, &reg_addr, 1, DEFAULT_TIMEOUT);
    if(status != HAL_OK){
        return status;
    }
    return HAL_I2C_Master_Receive(&hi2c1, device_addr<<1, reg_data, len, DEFAULT_TIMEOUT);
}
...
BME68X_INTF_RET_TYPE bme68x_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    return HAL_I2C_Master_Transmit(&hi2c1, device_addr<<1, buf, (uint16_t)len + 1, DEFAULT_TIMEOUT);
}
