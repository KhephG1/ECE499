...
void setup() {
  ...
  //initialize the gateway screen
    Serial.println("LoRa Gateway Starting...");
    display.println("      ECE 499 Gateway");
    display.println("Last Received From:");
    display.println("Last Node Battery:");
    display.println("Last Node RSSI:");
...
void loop() {
  std::string title = "      ECE 499 Gateway";
  std::string last_node_id = "Last Received From:";
  std::string last_node_battery = "Last Node Battery:";
  std::string last_node_rssi = "Last Node RSSI:";
    ...
    if (status == RADIOLIB_ERR_NONE) {
        ...
        Serial.printf(
            ...
            (sensors.adcVoltage / 100.0f) * 1.227,
            rssi,
            snr,
            sensors.deviceID
        );
    ...
    //update display
    std::string string_id = std::to_string(sensors.deviceID); 
    std::string string_battery = std::to_string(sensors.adcVoltage * 1.227 / 100.0f);
    std::string string_rssi = std::to_string(rssi);
    string_battery.resize(string_battery.size() - 4);
    string_rssi.resize(string_rssi.size() - 4);
    last_node_id += " " + string_id;
    last_node_battery += " " + string_battery;
    last_node_rssi += + " " + string_rssi; 
    // Resume listening
    radio.startReceive();
    //render display
    display.println();
    display.println(last_node_id.c_str());
    display.println(last_node_battery.c_str());
    display.println(last_node_rssi.c_str());
    //pad the screen so it is properly aligned
    display.println();
